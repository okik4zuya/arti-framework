...
"it's submitted" — four skills, a set of export tools, and a local dashboard.
...
| Skills don't appear in Claude Code | Restart Claude Code, then check `~/.claude/skills/` contains the four `ARTi-*` entries. |
...
Delete `~/.arti`, the four `ARTi-*` entries in `~/.claude/skills/`, and the Desktop icon.
...
```
~/.arti/
  skills/     the four ARTi-* skills (linked into ~/.claude/skills/)

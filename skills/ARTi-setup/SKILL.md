...
description: >
  ...
  `~/.arti/researcher-profile.md` doesn't exist yet.
---
...
2. Create the researcher-content subfolders under `~/.arti` if missing: `idea-bank.md`,
   ...
   first time ARTi-writing needs one. Also seed two short, imperative files, header-only:
   - `working-preferences.md` — one rule per line, `- **<rule as an instruction>** — (since
     YYYY-MM-DD, session N)`, grouped by output mechanics / interaction style / file and naming
     conventions / session-end ritual. Read at the start of every ARTi session. Written to in the
     same turn a researcher corrects Claude on *how* to do something — never deferred to session
     end.
   - `ebook-notes.md` — two sections: a low-ceremony chapter/topic idea dump, and a micro-flow
     catalog (name · trigger · steps as performed · friction points · time cost · tooling
     opportunity). Once a recurring loop repeats a second time, write it up here.
3. If `~/.arti/researcher-profile.md` doesn't exist, run the interview: ask the researcher directly
   ...
   (Positioning Line) is left blank — ARTi-idea writes it later, on first Idea Bank entry.
...
- Follow `references/project-scaffold-template.md` to create the new project's standard folder
  boilerplate — `idea/`, `writing/`, `literature/`, `data/`, `figures/`, `submission/` — plus a
  `memory/` folder holding just `MEMORY.md` and `todo-list.md`, and a root `CLAUDE.md`.
...
- Add a new row for this project to `~/.arti/progress-index.md` (stage: "Not started", status:
